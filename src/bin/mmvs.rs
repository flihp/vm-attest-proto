...
fn variance_classic(data: &Data, mean: u32) -> Result<u32> {
    ...
    let variance = variance / data.count as u128 - 1;
    Ok(variance as u32)
}
...
fn main() -> Result<()> {
    ...
    let mean = mean_classic(&data).context("calculate mean from dataset")?;
    println!("classic mean: {mean}");

    let variance = variance_classic(&data, mean)
        .context("calculate variance from dataset")?;
    println!("classic variance: {variance}");
    let sqrt = f64::from(variance).sqrt();
    println!("classic standard deviation: {}", sqrt);
